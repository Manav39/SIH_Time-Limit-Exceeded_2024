...
const ViewTopic = () => {
  ...
  useEffect(() => {
    if (topic) {
      axios
        .post("http://localhost:8088/auth/topiccomments", {
          topic_id: topic.id,
        })
        .then((response) => {
  ...
  const handleSave = async (id) => {
    try {
      const response = await axios.put(
        `http://localhost:8088/auth/view_forum/${id}`,
        {
          comment: newComment,
        }
      );
  ...
  const handleCommentSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8088/auth/view_forum",
        {
          c: newComment,
          user_id: userid,
          topic_id: topic.id,
              ...
              {topic && (
                <div className="row col-md-12 mb-2 justify-content-center">
                  <span className="badge badge-primary px-3 pt-1 pb-1">
                    <b>
                      <i>Topic Created by: {topic.created_by}</i>
                    </b>
                  </span>
                </div>
              )}
              ...
              {comments.map((c, index) => (
                <div className="form-group comment" key={index}>
                  {editingCommentId === c.id ? (
                    ...
                    <>
                      {isLoggedIn &&
                        (c.user_id == localStorage.getItem("user_id") ||
                          isAdmin) && (
