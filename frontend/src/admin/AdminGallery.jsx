...
const AdminGallery = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8088/auth/gallery")
      .then((res) => {
        setGallery(res.data);
      })
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8088/auth/gallery",
        formData,
        {
          headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        `http://localhost:8088/auth/gallery/${id}`
      );
                    ...
                    {gallery &&
                      gallery.map((g, index) => (
                        <tr key={index}>
                          ...
                          <td>
                            <img
                              src={`http://localhost:8088/${g.image_path}`}
                              className="gimg"
                              alt="img"
                            />
                          </td>
                          ...
                          >
                            <div className="d-flex  ">
                              <button
                                onClick={() =>
                                  handleEdit(g.image_path, g.about, g.id)
                                }
